...
public class SearchKeywordParser {
    ...
    // Pattern storage
    private static final Pattern STORAGE_PATTERN = Pattern.compile("\\b(\\d+)\\s*(gb|tb)\\b", Pattern.CASE_INSENSITIVE);

    // Pattern RAM:
    private static final Pattern RAM_PREFIX_PATTERN = Pattern.compile("\\bram\\s*(\\d+)(?:\\s*gb)?\\b",
            Pattern.CASE_INSENSITIVE);
    private static final Pattern RAM_SUFFIX_PATTERN = Pattern.compile("\\b(\\d+)(?:\\s*gb)?\\s*ram\\b",
            Pattern.CASE_INSENSITIVE);
